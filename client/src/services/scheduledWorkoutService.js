...
export const startScheduledWorkout = async (scheduledWorkoutId) => {
  try {
    ...
    const response = await api.get(`/api/scheduled-workouts/${scheduledWorkoutId}`);
    const scheduledWorkout = response.data.data;
    
    // Get the workout details
    const workoutResponse = await api.get(`/api/workouts/${scheduledWorkout.workoutId._id}`);
...
export const completeScheduledWorkout = async (scheduledWorkoutId, workoutSessionId) => {
  try {
    // Update the scheduled workout to mark it as completed
    const response = await api.put(`/api/scheduled-workouts/${scheduledWorkoutId}/complete`, {
      workoutSessionId
    });
