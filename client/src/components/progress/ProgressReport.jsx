...
const ProgressReport = () => {
  ...
  const fetchReportData = async () => {
    try {
      ...
      const filteredMeasurements = measurementsRes.data.data.filter(item => 
        isWithinInterval(new Date(item.date), { start: dateRange.start, end: dateRange.end })
      );
      ...
      const filteredGoals = goalsRes.data.data.filter(item => 
        item.status === 'active' || 
        (item.status === 'completed' && isWithinInterval(new Date(item.targetDate), { start: dateRange.start, end: dateRange.end }))
      );
      ...
      // Sort data by date
      const sortedMeasurements = filteredMeasurements.sort((a, b) => new Date(a.date) - new Date(b.date));
      const sortedWorkouts = filteredWorkouts.sort((a, b) => new Date(a.date) - new Date(b.date));
      const sortedNutrition = filteredNutrition.sort((a, b) => new Date(a.date) - new Date(b.date));
  ...
  return (
    <div className="space-y-6">
      <Card title="Progress Report" subtitle="Generate comprehensive reports of your fitness journey">
        <div className="flex flex-col md:flex-row md:justify-between md:items-center mb-6 space-y-4 md:space-y-0">
          {/* Report period selector */}
          <div className="flex items-center space-x-3">
            <span className="text-sm text-gray-500 dark:text-gray-400">Report period:</span>
            <select
              value={reportPeriod}
        ...
        {loading ? (
          <div className="text-center py-16">
            ...
            <p className="mt-3 text-sm text-gray-600 dark:text-gray-400">Loading report data...</p>
          </div>
        ) : error ? (
          <div className="text-center py-16">
          ...
          <div className="space-y-8">
            <div className="border border-gray-200 dark:border-gray-700 rounded-lg p-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <div className="bg-white dark:bg-gray-800 p-4 rounded-lg border border-gray-200 dark:border-gray-700 shadow-sm">
                  <div className="text-sm font-medium text-gray-500 dark:text-gray-400">Total Workouts</div>
                  <div className="mt-1 text-3xl font-bold text-gray-900 dark:text-white">{workouts.length}</div>
                </div>
                ...
                <div className="bg-white dark:bg-gray-800 p-4 rounded-lg border border-gray-200 dark:border-gray-700 shadow-sm">
                  ...
                  <div className="mt-1 text-3xl font-bold text-gray-900 dark:text-white">
                    {Math.round(workouts.reduce((total, workout) => total + (workout.duration || 0), 0))} <span className="text-lg font-normal">min</span>
                  </div>
                </div>
                ...
                <div className="bg-white dark:bg-gray-800 p-4 rounded-lg border border-gray-200 dark:border-gray-700 shadow-sm">
                  ...
                  <div className="mt-1 text-3xl font-bold text-gray-900 dark:text-white">
                    {Math.round(workouts.reduce((total, workout) => total + (workout.caloriesBurned || 0), 0))} <span className="text-lg font-normal">kcal</span>
                  </div>
                </div>
                
                <div className="bg-white dark:bg-gray-800 p-4 rounded-lg border border-gray-200 dark:border-gray-700 shadow-sm">
                  <div className="text-sm font-medium text-gray-500 dark:text-gray-400">Goals Completed</div>
                  <div className="mt-1 text-3xl font-bold text-gray-900 dark:text-white">
                ...
                <div className="overflow-x-auto">
                  <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                      {['weight', 'bodyFat', 'chest', 'waist', 'hips', 'arms', 'thighs'].map(metric => {
                        const firstMeasurement = measurements[0];
                        const lastMeasurement = measurements[measurements.length - 1];
                        
                        if (firstMeasurement[metric] === undefined || lastMeasurement[metric] === undefined) {
                          return null;
                        }
                        
                        const change = lastMeasurement[metric] - firstMeasurement[metric];
                        const changeFormatted = change.toFixed(1);
              ...
              <div className="border border-gray-200 dark:border-gray-700 rounded-lg p-4">
                ...
                <div className="space-y-4">
                  {goals.slice(0, 3).map(goal => {
                    ...
                    return (
                      <div key={goal._id} className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-4">
            ...
            <div className="text-sm text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-gray-800 p-4 rounded-lg border border-gray-200 dark:border-gray-700">
              <p>
                <span className="font-semibold">About this report:</span> This summary shows your progress for {getDateRange().label}.
                Download the full PDF report for more details and visualizations.
              </p>
            </div>
          </div>
        )}
