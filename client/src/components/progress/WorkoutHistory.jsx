...
const WorkoutHistory = () => {
  ...
  const fetchWorkoutHistory = async () => {
    try {
      ...
      if (dateRange === '1m') {
        filteredWorkouts = response.data.data.filter(w => 
          new Date(w.date) >= subMonths(now, 1)
        );
      } else if (dateRange === '3m') {
        filteredWorkouts = response.data.data.filter(w => 
          new Date(w.date) >= subMonths(now, 3)
        );
      } else if (dateRange === '6m') {
        filteredWorkouts = response.data.data.filter(w => 
          new Date(w.date) >= subMonths(now, 6)
        );
      } else if (dateRange === '1y') {
        filteredWorkouts = response.data.data.filter(w => 
          new Date(w.date) >= subMonths(now, 12)
        );
      }
  ...
  const getWorkoutStats = () => {
    ...
    const workoutDates = [...new Set(workouts.map(w => format(new Date(w.date), 'yyyy-MM-dd')))].sort();
  ...
  return (
    <div className="space-y-6">
      <Card title="Workout History" subtitle="Track your workout consistency and performance">
