...
const MeasurementTrends = () => {
  ...
  const fetchMeasurements = async () => {
    try {
      ...
      if (dateRange === '1m') {
        filteredMeasurements = response.data.data.filter(
          m => new Date(m.date) >= subMonths(now, 1)
        );
      } else if (dateRange === '3m') {
        filteredMeasurements = response.data.data.filter(
          m => new Date(m.date) >= subMonths(now, 3)
        );
      } else if (dateRange === '6m') {
        filteredMeasurements = response.data.data.filter(
          m => new Date(m.date) >= subMonths(now, 6)
        );
      } else if (dateRange === '1y') {
        filteredMeasurements = response.data.data.filter(
          m => new Date(m.date) >= subMonths(now, 12)
        );
      }
  ...
  const calculateMetricStats = data => {
    ...
    availableMetrics.forEach(metric => {
      ...
      const validData = data.filter(
        m => m[metricId] !== null && m[metricId] !== undefined
      );
  ...
  return (
    <div className='space-y-6'>
      <Card
        ...
        subtitle='Track how your body measurements change over time'
      >
