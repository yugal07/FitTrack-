...
const PhotoComparison = () => {
  ...
  useEffect(() => {
    if (beforePhoto && afterPhoto && beforeImgRef.current && afterImgRef.current && comparisonRef.current && sliderRef.current) {
      initComparison();
    }
    ...
    try {
      ...
      if (sortedPhotos.length >= 2) {
        if (selectedCategory !== 'all') {
          const categoryPhotos = sortedPhotos.filter(p => p.category === selectedCategory);
  ...
  return (
    <Card title="Photo Comparison" subtitle="Compare your progress photos over time">
      ...
      {loading ? (
        <div className="flex items-center justify-center h-64">
          ...
          <p className="mt-3 text-sm text-gray-600 dark:text-gray-400">Loading photos...</p>
        </div>
      ) : error ? (
        ...
        <div className="text-center py-10">
          ...
          <p className="text-gray-500 dark:text-gray-400 mb-4">
            You need at least two progress photos to make a comparison. Add more photos to get started.
          </p>
          <Button
            variant="primary"
        ...
        <div>
            ...
            {beforePhoto && afterPhoto ? (
              <div className="space-y-4">
                ...
                <div className="p-4 bg-gray-50 dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700">
                  <h4 className="font-medium text-gray-900 dark:text-white mb-2">Comparison Details</h4>
                  
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <p className="text-sm text-gray-600 dark:text-gray-400">
                        <span className="font-medium">Time difference:</span> {
                          formatTimeDifference(new Date(beforePhoto.date), new Date(afterPhoto.date))
                        }
                      </p>
                      
                      <p className="text-sm text-gray-600 dark:text-gray-400 mt-2">
                        <span className="font-medium">Category:</span> {
                          beforePhoto.category.charAt(0).toUpperCase() + beforePhoto.category.slice(1)
                        }
                      </p>
                    </div>
                    ...
                    <div className="space-y-2">
                      {beforePhoto.notes && (
                        <p className="text-sm text-gray-600 dark:text-gray-400">
                          <span className="font-medium">Before notes:</span> {beforePhoto.notes}
                        </p>
                      )}
                      
                      {afterPhoto.notes && (
                        <p className="text-sm text-gray-600 dark:text-gray-400">
                          <span className="font-medium">After notes:</span> {afterPhoto.notes}
                        </p>
                      )}
                    </div>
                  </div>
              ...
              <div className="flex flex-col md:flex-row gap-6 bg-gray-50 dark:bg-gray-800 rounded-lg p-6 border border-gray-200 dark:border-gray-700">
                ...
                <div className="md:w-1/2">
                  <h4 className="text-base font-medium text-gray-800 dark:text-gray-200 mb-3">Select Before Photo</h4>
                ...
                <div className="md:w-1/2 mt-6 md:mt-0">
                  <h4 className="text-base font-medium text-gray-800 dark:text-gray-200 mb-3">Select After Photo</h4>
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
          ...
          <div className="mt-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {filteredPhotos.map(photo => (
                <div key={photo._id} className="border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden">
                  <div className="relative">
                    <img 
                      src={photo.photoUrl} 
