...
const GoalStatistics = ({ goals, loading }) => {
  ...
  if (goals.length === 0) {
    return (
      <Card
        ...
        subtitle='Start creating goals to see your statistics'
      >
  ...
  return (
    <Card
      title='Goal Statistics'
      subtitle='Overview of your goals and progress'
    >
      <div className='py-4'>
          ...
          <div className='bg-white dark:bg-gray-800 p-4 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700'>
            ...
            <div className='mt-3 space-y-2'>
              <div className='flex items-center'>
                <span
                  className={`inline-block w-3 h-3 ${getStatusColor('active')} rounded-full mr-2`}
                ></span>
              ...
              <div className='flex items-center'>
                <span
                  className={`inline-block w-3 h-3 ${getStatusColor('completed')} rounded-full mr-2`}
                ></span>
                <span className='text-sm text-gray-600 dark:text-gray-300'>
                  Completed
                </span>
              ...
              <div className='flex items-center'>
                <span
                  className={`inline-block w-3 h-3 ${getStatusColor('abandoned')} rounded-full mr-2`}
                ></span>
                <span className='text-sm text-gray-600 dark:text-gray-300'>
                  Abandoned
                </span>
            ...
            {stats.closestGoal ? (
              <div className='mt-2'>
                <div className='flex items-center mb-1'>
                  <div
                    className={`w-2 h-2 ${stats.daysUntilClosest <= 7 ? 'bg-red-500' : 'bg-yellow-500'} rounded-full mr-2`}
                  ></div>
                ...
                <div className='w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2.5'>
                  <div
                    ...
                    style={{
                      width: `${Math.min(100, Math.round((stats.closestGoal.currentValue / stats.closestGoal.targetValue) * 100))}%`,
                    }}
                  ></div>
                </div>
                <p className='mt-1 text-xs text-gray-500 dark:text-gray-400'>
                  {stats.closestGoal.currentValue} /{' '}
                  {stats.closestGoal.targetValue} {stats.closestGoal.unit}
                </p>
              </div>
            ) : (
