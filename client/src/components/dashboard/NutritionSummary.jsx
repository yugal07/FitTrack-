...
const NutritionSummary = ({ nutritionData }) => {
  if (!nutritionData) {
    return (
      <div className="bg-white dark:bg-gray-800 shadow rounded-lg p-6">
        <h2 className="text-lg font-medium text-gray-900 dark:text-white">Today's Nutrition</h2>
        <div className="mt-4 text-center py-8">
          <p className="text-gray-500 dark:text-gray-400">No nutrition data logged today.</p>
          <button 
  ...
  const caloriesPercentage = Math.min(100, Math.round((totalCalories / targetCalories) * 100));
  ...
  const waterPercentage = Math.min(100, Math.round((waterIntake / targetWater) * 100));
  ...
  return (
    <div className="bg-white dark:bg-gray-800 shadow rounded-lg p-6">
      ...
      <div className="mt-4 space-y-4">
        ...
        {/* Macros */}
        <div>
          <h3 className="text-sm font-medium text-gray-700 dark:text-gray-300">Macronutrients</h3>
        ...
        <div>
          <div className="flex items-center justify-between">
            ...
            <span className="text-sm text-gray-500 dark:text-gray-400">
              {(waterIntake / 1000).toFixed(1)} / {(targetWater / 1000).toFixed(1)} L
            </span>
          </div>
          <div className="mt-2 w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2.5">
            <div 
      ...
      <div className="mt-6 text-center">
        <a href="/nutrition" className="text-sm font-medium text-indigo-600 dark:text-indigo-400 hover:text-indigo-500 dark:hover:text-indigo-300">
