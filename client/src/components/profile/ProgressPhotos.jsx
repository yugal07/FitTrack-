  ...
  const handleSelectPhoto = (photo, type) => {
    if (type === 'before') {
      ...
      if (!afterPhoto) {
        ...
        const newerPhotos = photos.filter(p => 
          new Date(p.date) > new Date(photo.date) && p.category === photo.category
        );
      ...
      if (!beforePhoto) {
        ...
        const olderPhotos = photos.filter(p => 
          new Date(p.date) < new Date(photo.date) && p.category === photo.category
        );
    ...
    >
      ...
      {compareMode ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden">
              <div className="bg-gray-50 dark:bg-gray-800 px-4 py-2 border-b border-gray-200 dark:border-gray-700">
                <h4 className="font-medium text-gray-700 dark:text-gray-300">Before</h4>
              </div>
            ...
            <div className="border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden">
              <div className="bg-gray-50 dark:bg-gray-800 px-4 py-2 border-b border-gray-200 dark:border-gray-700">
                <h4 className="font-medium text-gray-700 dark:text-gray-300">After</h4>
              </div>
          ...
          {beforePhoto && afterPhoto && (
            <div className="mt-4 p-4 bg-gray-50 dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700">
              ...
              {(beforePhoto.notes || afterPhoto.notes) && (
                <div className="mt-2">
                  <p className="text-sm text-gray-600 dark:text-gray-300">
                    <span className="font-medium">Before notes:</span> {beforePhoto.notes || 'None'}
                  </p>
                  <p className="text-sm text-gray-600 dark:text-gray-300">
                    <span className="font-medium">After notes:</span> {afterPhoto.notes || 'None'}
                  </p>
                </div>
              )}
            </div>
        ...
        <div className="text-center py-12 bg-gray-50 dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700">
          ...
          <h3 className="mt-4 text-lg font-medium text-gray-900 dark:text-white">No progress photos yet</h3>
          <p className="mt-2 text-sm text-gray-500 dark:text-gray-400">Start tracking your fitness journey by adding your first progress photo.</p>
          <button
            type="button"
            onClick={() => setShowForm(true)}
                  ...
                  >
                    ...
                    <div className="absolute bottom-0 left-0 right-0 bg-black bg-opacity-60 text-white p-2">
                      <div className="flex justify-between items-center">
                        <span className="text-sm">{format(new Date(photo.date), 'MMM d, yyyy')}</span>
