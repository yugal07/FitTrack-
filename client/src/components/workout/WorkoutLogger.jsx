...
const WorkoutLogger = () => {
  ...
  const addExercise = (exerciseId) => {
    ...
    if (exercise && !selectedExercises.find(ex => ex.exerciseId === exerciseId)) {
  ...
  const onSubmit = async (formData) => {
    try {
      ...
      try {
        ...
        sessionResponse = await api.post('/api/workout-sessions', workoutSessionData);
      } catch (error) {
        if (error.response?.status === 404) {
          console.log('Trying alternative endpoint...');
          // Try alternative endpoint names
          try {
            sessionResponse = await api.post('/api/workouts/session', workoutSessionData);
          } catch (altError) {
            try {
              sessionResponse = await api.post('/api/sessions', workoutSessionData);
            } catch (finalError) {
              throw new Error('Workout session endpoint not found. Please set up /api/workout-sessions');
            }
          }
        } else {
          throw error;
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-gray-900">
        <div className="text-center">
          ...
          <p className="mt-3 text-sm text-gray-600 dark:text-gray-400">Loading workout...</p>
        </div>
      </div>
    );
  }
  ...
  const completedExercises = selectedExercises.filter(ex => ex.isCompleted).length;
            ...
            <div className="bg-white dark:bg-gray-800 shadow rounded-lg p-6">
              ...
              <div className="mb-4">
                ...
                >
                  ...
                  {availableExercises
                    .filter(ex => !selectedExercises.find(sel => sel.exerciseId === ex._id))
                    .map(exercise => (
              ...
              {selectedExercises.length === 0 ? (
                ...
                <div className="space-y-3">
                  {selectedExercises.map((exercise, index) => (
                    <div key={`selected-${exercise.exerciseId}-${index}`} className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
                      <div>
          ...
          <div className="space-y-6">
            ...
            {currentExercise && (
              <ExerciseTracker
                ...
                onCompleteExercise={() => markExerciseComplete(currentExerciseIndex)}
              />
            )}
...
const ExerciseTracker = ({ exercise, exerciseIndex, onAddSet, onCompleteExercise }) => {
        ...
        <div className="mb-6">
          ...
          <div className="space-y-2">
            {exercise.completedSets.map((set, index) => (
              <div key={`set-${index}-${set.timestamp || index}`} className="flex justify-between items-center p-2 bg-gray-50 dark:bg-gray-700 rounded">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => setSetData(prev => ({ ...prev, duration: e.target.value }))}
              className="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
              placeholder="60"
            />
          </div>
          <div>
            ...
            <input
              ...
              onChange={(e) => setSetData(prev => ({ ...prev, restTime: e.target.value }))}
              className="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
              placeholder="90"
            />
          </div>
